module.exports = function (RED) {
    ...
    var loginToTierionAndVerifyHash = function (receiptId, node, callback) {
        ...
        if (globalTierionUserAccessTokenExpireTime != undefined && globalTierionUserAccessTokenExpireTime > Date.now()) {
            if (node.config.enabledebug == 'true') {
                node.warn('Already logged in to Tierion');
            }
            ...
            if (node.config.enabledebug == 'true') {
                node.warn('Not already logged in to Tierion, so doing that now');
            }
    ...
    function blockchainVerifyNode(config) {
        ...
        this.on('input', function (msg) {
            ...
            loginToTierionAndVerifyHash(msg.payload.tierionReceiptId, node, function (jsonDataReturned) {
                ...
                if (node.config.enabledebug == 'true') {
                    node.warn('receipt data is: ' + JSON.stringify(msg.payload.tierionReceipt));
                }
                ...
                if (msg.payload.tierionReceipt.targetHash == hashValHex || hashVal == '') {
                    if (node.config.enabledebug == 'true') {
                        node.warn('blockchain-verify: hash = targetHash');
                    }
